...
class VideoListDataModel extends ChangeNotifier {
  List data = [
    {
      "description":
...
class VideoPlayerModel extends ChangeNotifier {
  ...
  void videoLoadedInController(String url) {
    loading = true;
    notifyListeners();
    controller = VideoPlayerController.networkUrl(Uri.parse(url))
      ..initialize().then((_) {
        // Ensure the first frame is shown after the video is initialized, even before the play button has been pressed.
        if (controller.value.duration.inHours == 0) {
          isHourZero = true;
        }
        if (controller.value.duration.inMinutes == 0) {
          isMinutesZero = true;
        }
        controller.addListener(updateSeeker);
        controller.play();
        loading = false;
        notifyListeners();
      });
  ...
  void seeker(value) {
    ...
    controller.seekTo(position).then((value) => {actionsSheet(2, true)});
  }
  ...
  void rotationMech(bool isPortrait) async {
    if (!isPortrait) {
